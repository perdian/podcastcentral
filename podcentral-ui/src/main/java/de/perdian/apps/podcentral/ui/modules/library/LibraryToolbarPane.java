...
public class LibraryToolbarPane extends BorderPane {
    ...
    public LibraryToolbarPane(JobScheduler jobScheduler, Library library, Localization localization) {
        ...
        refreshFeedsButton.setOnAction(new FeedRefreshEventHandler(() -> library.getFeeds().stream().map(feed -> feed.getUrl().getValue()).collect(Collectors.toSet()), () -> {}, jobScheduler, library, localization));

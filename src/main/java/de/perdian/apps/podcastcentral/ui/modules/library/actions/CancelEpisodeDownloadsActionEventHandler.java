...
public class CancelEpisodeDownloadsActionEventHandler implements EventHandler<ActionEvent> {
    ...
    public CancelEpisodeDownloadsActionEventHandler(Supplier<List<Episode>> episodesSupplier, EpisodeDownloader episodeDownloader, BackgroundTaskExecutor backgroundTaskExecutor, Localization localization) {
        ...
        if (!episodes.isEmpty()) {
            this.getBackgroundTaskExecutor().execute(this.getLocalization().schedulingEpisodeDownloads(), progress -> {
                for (int i=0; i < episodes.size(); i++) {
                    ...
                    this.getEpisodeDownloader().cancelDownload(episodes.get(i));
                }
            });
        }
    }
